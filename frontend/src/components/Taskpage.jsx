...
const TaskPage = () => {
  ...
  useEffect(() => {
    ...
    if (dateFromQuery) {
      ...
      // If only taskId is provided, fetch all tasks and highlight the specific one
      fetchAllTasksAndHighlight(taskIdFromQuery);
    }
  }, [location, userId]);
  ...
  // New function to fetch all tasks and highlight specific task
  const fetchAllTasksAndHighlight = async (taskId) => {
    setLoading(true);
    setErrorMessage("");
    
    try {
      // First, try to find the task by ID to get its date
      const taskResponse = await fetch(`${ipadr}/get_task_by_id/${taskId}`);
      
      if (taskResponse.ok) {
        const taskData = await taskResponse.json();
        const taskDate = taskData.date; // Assuming the task has a date field
        
        if (taskDate) {
          setDate(taskDate);
          await fetchTasks(userId, taskDate);
          
          // Highlight the specific task after a short delay
          setTimeout(() => {
        ...
        // If task not found by ID, try fetching today's tasks as fallback
        const today = new Date();
        const day = String(today.getDate()).padStart(2, '0');
        const month = String(today.getMonth() + 1).padStart(2, '0');
        const year = today.getFullYear();
        ...
        setDate(todayDate);
        await fetchTasks(userId, todayDate);
        toast.info("Task details not found. Showing today's tasks.");
      }
    } catch (error) {
    ...
    return (
      <ul className="space-y-4">
        {tasks.map((task, index) => (
          <li
            key={index}
            id={`task-${task._id || task.taskid}`}
            className="flex justify-between items-center bg-white p-4 rounded-lg shadow-sm border border-gray-200 w-full transition-all duration-300"
          >
