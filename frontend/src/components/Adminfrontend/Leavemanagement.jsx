  ...
  const data = [
    {
      ...
      icon: (
        <FontAwesomeIcon
          icon={faUserClock}
          className="text-6xl mb-6 text-zinc-700"
        />
      ),
      button: 'View now',
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {data.map((item, index) => (
              ...
              >
                ...
                {LS.get('isadmin') === true ? (
                  <Link to={item.link1}>
                    <button className="bg-blue-500 hover:bg-[#b7c6df80] hover:text-black text-white font-poppins px-4 py-2 mt-3 rounded-full shadow-lg">
                      {item.button}
                    </button>
                  </Link>
                ) : (
